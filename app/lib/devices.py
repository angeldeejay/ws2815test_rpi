import random, time, socket, binascii
...
class Sonoff:
    ...
    def __publish(self, topic, message=None):
        if self.connected:
            ...
            print(f"Failed to send message `{message}` to topic {topic}: {result}", flush=True)
        return False

    def __repr__(self):
        return self.__class__.__name__ + f'<broker: {self.broker}, port: {self.port}, device: {self.device}, connected: {self.connected}, on: {self.on}>'
